...
const renameFiles = [
  ["src/library.ts", "src/await-simple.ts"],
  ["test/library.test.ts", "test/await-simple.test.ts"]
]
...
function modifyContents(libraryName: string, username: string, usermail: string) {
  ...
  try {
    const changes = replace.sync({
      files,
      from: [/await-simple/g, /--username--/g, /--usermail--/g],
...
function renameItems(libraryName: string) {
  ...
  renameFiles.forEach(function(files) {
    ...
    let newFilename = files[1].replace(/await-simple/g, libraryName)
    mv(
